...
![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/1f14c5b9-bb6e-4061-af26-ab764637df2e)
*<h5>The first 3 principal components accounted for 47.6% of total variance. All puzzles from Jan. 1, 2018- Jan. 25, 2024 were included in this analysis (N=2,215).*
###
...
![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/7827f7eb-9638-4d70-af54-b5f9f322e2b1)
...
![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/8a0de6da-b802-45a8-8e8a-529ed21d386c)
...
![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/14f717d4-793e-47fd-a6dc-742496f26b80)
...
*<h5>Correlation heatmaps derived from N=1,274 15x15 (left panel) and N=212 21x21 (right panel) puzzles solved by GMS from 2020-2024.*
...
![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/ef466886-d34a-44c8-9a91-24578755b7a6)
*<h5>Global Median Solver solve times (GMSTs) and '# Answers' had a moderately strong negative correlation for 15x15 puzzles (r= -.58).<br>*

*More answers typically meant shorter answers (see correlation matrices above), and shorter answers tended to be more common/easier answers (see 'Average Answer Length' and 'Freshness Factor' analyses below). Aligned with this relationship, the FDP for this feature shows that the toughest puzzle days (Fri and Sat) tended to have the fewest answers. Additionally, moderately strong reverse sign (positive) correlations were seen for several early week puzzle days (Mon and Wed). This finding suggests that, below a particular per-clue/answer difficulty threshold mostly only attained in later week (Thu-Sat) puzzles, the time penalty incurred by having to read relatively more clues is greater than the time savings from encountering relatively fewer longer answers. Additional support for the tradeoff comes from the moderate negative correlation seen for Saturday; ie, when long answers are tough answers this outweighs having to deal with relatively fewer clues to solve a given puzzle.*


**<h4>Figure 9. Number of Open Squares**

![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/c7ac6936-4d75-4397-8034-229192808364)
*<h5>GMSTs and '# Open Squares' had a borderline strong positive correlation for 15x15 puzzles (r= .59).<br>*

*'# Open Squares' is a proprietary measure from XWord Info that counts all white squares that are *not* bordered by black squares. '# Open Squares' was strongly positively correlated to 'Average Answer Length' (see matrices above), so it makes sense that a greater '# Open Squares' was also positively correlated with solve times. The FDP shows that the most difficult puzzle days (Fri and Sat) had a rightward shift in '# Open Squares' relative to the easier 15x15 puzzle days. A large amount of the overall 15x15 puzzles positive correlation appears to be accounted for by puzzles, nearly exclusively on those two most difficult days, with >~80 '# Open Squares'. The positive correlation for Saturday, approximating the strength of the overall 15x15 positive correlation, is an indication that this feature will likely have some independent predictive value in the modeling phase (see prefacing text for this section).* 


**<h4>Figure 10. Number of Black Squares**

![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/e882aea7-2649-45fe-be8d-5ebe360bdffb)
...
![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/b4fddf90-5626-4d1f-909a-04902c805900)
*<h5>GMSTs and 'Average Answer Length' had a strong positive correlation for 15x15 puzzles (r= .70).<br>*

*The moderately strong overall correlation for 15x15 puzzles for GMS appears to be largely driven by the two most difficult puzzle days (Fri and Sat) being strongly right-shifted in the FDP. Several early-week puzzle days (Mon and Wed) stood out among in showing weak-to-nonexistant correlations bordering on sign reversals relative to the overall 15x15 puzzle correlation. Given that this feature was strongly negatively correlated with '# Answers (see Figs. 7 and 8), the same logic may apply here as does there. Namely that longer answers that are relatively easy may increase solver speed by reducing the amount of clues needed for a solve without a counterbalancing 'difficulty penalty' that might occur with longer answers on later puzzle days. To this end, Saturday does indeed have a strong positive correlation across a wide range of feature values, with higher 'Average Answer Length' being associated with slower solves.*  


**<h4>Figure 12. Number of Cheater Squares**

![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/e9c5fefd-06c8-4857-8946-8226aaff756f)
*<h5>GMSTs and '# Cheater Squares' had a weak positive correlation for 15x15 puzzles (r= .21).<br>*
...
![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/b9170c75-e23f-483b-8f62-a83370417e42)
*<h5>GMSTs and '# Scrabble Illegal' had a weak positive correlation for 15x15 puzzles (r= .18).<br>*
...
![image](https://github.com/ursus-maritimus-714/NYT-XWord-EDA-Global-Median-Solver/assets/90933302/a60a1801-4bd2-41be-bebe-d819b6e87997)
...
*<h5>Only Sunday and Thursday had an appreciable '# Rebus Squares' for the set of GMS solves. Rebus squares are those that must be filled with more than one letter, number or symbol for a given puzzle to be solved. There were weak-to-moderate positive correlations for both Sunday (r=.23) and Thursday (r=.20). The directionality of the correlations does make intuitive sense, as both their existence and the "rules" for any given rebus can often take a little while to figure out. Additionally, they increase solve time by some degree simply by requiring additional fill and menu toggling relative to a non-rebus puzzle. One caveat here is that the very large number of 0 rebus puzzles, even on Sunday and Thursday, make the strength of these correlations hard to interpret (ie, these are not exactly continuous distributions).*<br>
