...
*It is noteworthy in comparison to the GMS that the lowest correlation between next raw solve times and RPB for both individual solvers (IS1 and IS2; see links to their analyses in the Introduction to this summary) was for Thursday (.15 for IS1). This is perhaps unsurprising, since Thursday had a large degree of heterogeneity, with nearly all puzzles on that day involving a gimmick or trick of some variety (including rebuses of various flavors; see Supp. Fig. 2). Thursday came in as the day with the second lowest correlation for the GMS as well (.35), so variability in performance on that puzzle day may generalize across the solver pool.*
