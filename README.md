...
**Figure 6** shows correlation scatterplots of GMS past (relative to each individual puzzle date) performance versus a given constructor against GMS raw solve time % difference from day-specific 10-puzzle moving average for each puzzle. Plotting these normalized features against each other shows correlation between past performance against constructor(s) and performance on an individual puzzle in a way that controlls for both puzzle day difficulty and recent solve form of the solver. The left panel shows this correlation for all constructors, while the right panel includes only data for constructors who reached the >=5 puzzle threshold for inclusion in the heatmapping figure.      
